...
struct Cannon {
	RectangleShape unitCannon;
	bool isLookingLeft;

	Cannon(float px, float py, bool orientation){
		unitCannon.setSize(Vector2f(125, 75));
		unitCannon.setPosition(px, py);
		isLookingLeft = orientation; 
	}
};

struct Bullet {
	CircleShape unitBullet;
	bool isGoingLeft;

	Bullet(){
		unitBullet.setRadius(BULLET_SIZE);
		unitBullet.setPosition(900, 900); //out of bounds
		isGoingLeft = false;
	}
};
...
struct Character { //structure for the character/player with its own members and member functions
	...
	float left() {
		/*if (isLookingLeft) */return unit.getPosition().x; 
		// return unit.getPosition().x + unit.getSize().x; 
	} //returns left side of the player 
	float right() { 
		/*if (isLookingLeft)*/ return unit.getPosition().x + unit.getSize().x; 
		// return unit.getPosition().x; 
	} //returns right side of the player
...
void updatePosition(){ //update position of the player and the bullets
	...
	for(list<Bullet*>::iterator b = bullets.begin(); b != bullets.end(); ++b){ 
		if (isInsideWindow((**b).unitBullet.getPosition(), Vector2f((**b).unitBullet.getRadius(), (**b).unitBullet.getRadius()))) { 
			if ((**b).isGoingLeft) (*b)->unitBullet.move(-BULLET_SPEED, 0); 
			else {(*b)->unitBullet.move(BULLET_SPEED, 0); }
		} 
	}
}
...
void updateGrid(){ //update the uniform grid
	for(list<GridCell*>::iterator g = grid.begin(); g != grid.end(); ++g){
		...
		for(list<Bullet*>::iterator bu = bullets.begin(); bu != bullets.end(); ++bu){
			if ((**g).has((**bu).unitBullet.getGlobalBounds())) { (**g).bullets.push_back(*bu); }
		}
	}
}
...
void checkWorldCollision(){
	...
	//player-tile collision
		...
		if(!(**g).cannons.empty() && (**g).containsPlayer){ //if a cannon and the player is on the same grid cell
			for(vector<Cannon*>::iterator ca = (**g).cannons.begin(); ca != (**g).cannons.end() && gameState == 1; ++ca){
				if (player->has((**ca).unitCannon.getGlobalBounds())) { //this means they're colliding in some way
					Vector2f penetrationInfo = overlapCheck(player->top(), player->bottom(), 
						player->left(), player->right(), (**ca).unitCannon.getPosition().y, 
						(**ca).unitCannon.getPosition().y + (**ca).unitCannon.getSize().y, (**ca).unitCannon.getPosition().x, 
						(**ca).unitCannon.getPosition().x + (**ca).unitCannon.getSize().x);
		...
		if(!(**g).bullets.empty() && (**g).containsPlayer){ //if a bullet and the player is on the same grid cell
			for(vector<Bullet*>::iterator bu = (**g).bullets.begin(); bu != (**g).bullets.end() && gameState == 1; ++bu){
				if (player->has((**bu).unitBullet.getGlobalBounds())){ //this means they're colliding in some way 
...
bool cannonShoot(float secondsElapsed){
	if (secondsElapsed > 1.8) { //each bullet (is) constantly spawns/(fired) every 1.8 seconds, thus when it reaches this point, it will load another bullet
		Bullet* shootMe; 
		timeBeforePause = 0;
		for(list<Cannon*>::iterator ca = cannons.begin(); ca != cannons.end(); ++ca){
			bool found = false; //bullet initially pooled (not part of the screen)
			for(list<Bullet*>::iterator bu = bullets.begin(); bu != bullets.end() && !found; ++bu){
				if (!isInsideWindow((**bu).unitBullet.getPosition(), Vector2f((**bu).unitBullet.getRadius(), (**bu).unitBullet.getRadius()))) { //loading the bullet to the screen
					shootMe = *bu;
					Vector2f newPos = (**ca).unitCannon.getPosition();
					if ((**ca).isLookingLeft) {shootMe->isGoingLeft = true; newPos -= Vector2f(0, BULLET_SIZE*2);}
					else {shootMe->isGoingLeft = false; newPos += Vector2f(0, BULLET_SIZE*2);}
					shootMe->unitBullet.setPosition(newPos);
...
void initializeGrid(){
	...
	for(list<GridCell*>::iterator g = grid.begin(); g != grid.end(); ++g){
		...
		//put cannons
		for(list<Cannon*>::iterator ca = cannons.begin(); ca != cannons.end(); ++ca){
			if (isInsideWindow((**ca).unitCannon.getPosition(), (**ca).unitCannon.getSize())) {
				if ((**g).has((**ca).unitCannon.getGlobalBounds())) { (**g).cannons.push_back(*ca); }
			}
		}
	}
}
...
void initializeGame(){ 
	...
	cin >> c;
	for (int i = 0; i < c; i++){
		cin >> px >> py >> orientation;
		Cannon* c = new Cannon{px, py, orientation};
		c->unitCannon.setTexture(&(ts->cannonTexture));
		cannons.push_back(c);
	}
	//CREATE BULLETS (TO AVOID)
	for (int i = 0; i < MAX_BULLETS; i++){
		Bullet* b = new Bullet();
		b->unitBullet.setTexture(&(ts->cannonBallTexture));
		bullets.push_back(b);
	}
	//CREATE FLAG (TO WIN)
	cin >> px >> py;
...
int main(){
	...
	window.create(VideoMode(WINDOW_W, WINDOW_H), "Platformer", Style::Default, settings);
		...
		while(window.pollEvent(event)){
			switch(event.type){
				...
				switch(event.key.code){
					...
					if (gameState == 1) {
						moveLeft = true; 
/*						if (player->isLookingLeft) player->moveX(player->unit.getSize().x);
...
/*				if (player->isLookingLeft) player->unit.setScale(1, 1);
				else player->unit.setScale(-1, 1);*/
			...
			for(list<Bullet*>::iterator it = bullets.begin(); it != bullets.end(); ++it){ 
				if (isInsideWindow((**it).unitBullet.getPosition(), Vector2f((**it).unitBullet.getRadius(), (**it).unitBullet.getRadius()))) { window.draw((**it).unitBullet); } 
			} //bullet will only be drawn when it is already fired
			for(list<Cannon*>::iterator it = cannons.begin(); it != cannons.end(); ++it){ window.draw((**it).unitCannon); }
			player->drawPlayer();
			window.draw(*flag);

			if (gameState == 2) {window.draw(pauseScreen); clock.restart();} 
