...
void checkCollisionPerGrid(){
	for(list<vector<vector<RectangleShape*>>>::iterator g = grid.begin(); g != grid.end(); ++g){
		if (!(*g)[0].empty() && !(*g)[1].empty()){
			for(vector<RectangleShape*>::iterator br = (*g)[0].begin(); br != (*g)[0].end();){
				cout << "oh shit i can reach here" << endl;
				if ((*br) != NULL){
					for(vector<RectangleShape*>::iterator bu = (*g)[1].begin(); bu != (*g)[1].end();){
						if ((*bu) != NULL){
							//check for collision
							...
							bu = (*g)[1].erase(bu); //delete pointer insde the vector
						}
					}
				}
				else {
					br = (*g)[0].erase(br); //delete pointer insde the vector
				}
			}
		}
	}
...
void initializeObjects(){
	...
	//make all bullets
	for (int i = 0; i < BULLETS; i++) { makeNewBullet(); }
